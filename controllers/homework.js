...
function registerHomework(req, res) {
    ...
    connection.connect()
    connection.query('INSERT INTO tarea(nombre_tarea,fecha_entrega_tarea,tipo_entrega,id_curso,archivo) values(?,?,?,?,?)', [newHomework.nombre_tarea, newHomework.fecha_entrega, newHomework.tipo_entrega, newHomework.id_curso, newHomework.archivo], (err, result) => {
        if (err) {
            return res
                .status(200)
                .send({
